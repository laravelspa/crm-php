...
$pn = $_SESSION['pncancel'] = $_GET && $_GET['pn'];
include('../main/header1.php');
include('../main/database.php');

// All orders In Canceld Table
$stmt = $con->prepare("SELECT * FROM canceld");
$stmt->execute();
$countCancel = $stmt->rowCount();

// Group By Project Name
$stmt = $con->prepare("SELECT pname, count(*) FROM canceld GROUP BY pname");
$stmt->execute();
...
// Leads in Table Canceld
if (isset($pn)) {
  // Group By Employee ID
  $where = isset($pn) && $pn !== 'all' ? " WHERE pname ='" . $pn . "' GROUP BY emp_id" : ' GROUP BY emp_id';
  $stmt = $con->prepare("SELECT canceld.emp_id, admins.name, count(canceld.id) as count FROM canceld INNER JOIN admins ON canceld.emp_id = admins.id $where");
  $stmt->execute();
  $employee_group = $stmt->fetchAll(PDO::FETCH_ASSOC);

  // Group By Cancel Reason
  $where = isset($pn) && $pn !== 'all' ? ' WHERE pname ="' . $pn . '" AND (status ="changed his mind" OR status ="did not order" OR status= "aggressive" OR status= "expensive" OR status= "joke" OR status= "wrong number" OR status= "spam") GROUP BY status' : ' GROUP BY status';
  $stmt = $con->prepare('SELECT status, count(id) as count FROM canceld ' . $where);
  $stmt->execute();
...
<?php if (!isset($_GET['pn'])) { ?>
  <nav aria-label="breadcrumb">
    <ol class="breadcrumb bg-white">
      ...
      <li class="breadcrumb-item active"><a href="cancel.php">Canceld</a></li>
    </ol>
  </nav>
<?php } else { ?>
  <nav aria-label="breadcrumb">
    <ol class="breadcrumb bg-white">
      ...
      <li class="breadcrumb-item" aria-current="page">
        <a href="cancel.php?pn=<?php echo $pn ?>">
          <?php echo $pn; ?>
        </a>
      </li>
    </ol>
  ...
  <div class="col-md-3 col-sm-6 col-12">
    <div class="info-box">
      ...
      <div class="info-box-content">
        <span class="info-box-text"><a href="canceld.php?pn=all" style="text-decoration: none" class="<?php echo $_GET['pn'] === 'all' ? 'font-weight-bold' : ''; ?>">
            All</a></span>
        <span class="info-box-number"><?php echo $countCancel ?></span>
      </div>
      <!-- /.info-box-content -->
    </div>
    <!-- /.info-box -->
    ...
    foreach ($projects as $key => $value) { ?>
      ...
      <div class="col-md-3 col-sm-6 col-12">
        <div class="info-box">
          ...
          <div class="info-box-content">
            <span class="info-box-text"><a href="canceld.php?pn=<?php echo $key ?>" style="text-decoration: none" class="<?php echo $_GET['pn'] === $key ? 'font-weight-bold' : ''; ?>">
                <?php echo $key ?></a></span>
...
<div class="card">
  <div class="card-header border-transparent">
    <h3 class="card-title">Canceld</h3>
  ...
  <div class="card-body">
    <div class="float-right d-flex justify-content-around col-lg-4 col-md-12 col-sm-12">
      <button type="button" id="deleteAllButton" class="col-5 p-1 btn btn-outline-danger btn-sm" onclick="deleteAll('/delete.php', 'canceld')">
    ...
    <div class="d-flex flex-wrap align-items-center justify-content-center col-12 p-2">
      <form class="col-12" method="POST" id="filter_canceld">
        <div class="d-lg-flex d-md-block justify-content-lg-between">
          ...
          <button type="submit" class="col-lg-2 btn btn-sm btn-block btn-outline-primary"><i class="fas fa-filter"></i> Filter
          </button>
        </div>
      </form>
    </div>
      ...
      <table id="table_id" class="table table-hover table-bordered table-responsive" style="width:100%">
        <thead>
          <tr>
            ...
            <th style="min-width:20px">Phone</th>
            <th style="min-width:20px">Product</th>
            ...
            <th style="min-width:100px">Canceld at</th>
            <th style="min-width:10px">History</th>
          </tr>
        </thead>
        <tbody></tbody>
      </table>
    ...
    <div class="modal-content">
      ...
      <div class="modal-footer">
        <div class="form-group col-12 d-flex justify-content-between">
          ...
          <button id="submit" type="button" class="btn btn-sm btn-success" onclick="updateOrders('re_assign.php','canceld','Once Assaign Employee, This will disappearing!','You must choose any lead/leads','Your Canceld Order Not Changed!')">
            Save
          </button>
        </div>
      </div>
...
<script>
  getRecords('fetch_canceld.php');
  $(document).ready(function() {
    document.title = 'Dashboard - Canceld';
    ...
    $(document).on('submit', '#filter_canceld', function(e) {
      e.preventDefault();
      count++;
      if (count === 1) {
        ...
        getRecords('fetch_canceld.php', emp_id, status, date_first, date_last);
      }
      count = 0;
    })
    $('#employee').select2({
