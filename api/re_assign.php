...
if (isset($_POST['selected'], $_POST['employee_id'], $_POST['admin_id'])) {
  ...
  if ($spread !== '') {
    ...
    for ($i = count($leads) - 1; $i >= 0; $i--) {
      $lead_from = "origin1_" . $leads[$i]['id'] . "_" . $leads[$i]['lead_id'];
      $sql = "CALL add_project(:name, :phone, :address, :city, :pname, :prname, :prpieces,:prprice, :prcurrency, :emp_id, :status, :pending_comment, :pending_date, :pending_time, :doo, :dod, :created_at, :created_by, :db_id, :lead_from, :lead_id, :web_id, :added_at)";
      ...
      $id = $leads[$i]['id'];
      $web_id = $leads[$i]['web_id'];
      $added_at = $leads[$i]['add_date'];
      ...
      $stmt->bindParam("web_id", $web_id, PDO::PARAM_STR);
      $stmt->bindParam("added_at", $added_at, PDO::PARAM_STR);

      if ($stmt->execute()) {
        if ($i == 0) {
          $sql = "UPDATE terraleads SET deleted_at = NOW() WHERE id IN(" . $ids . ")";
          $stmt = $con->prepare($sql);
          if ($stmt->execute()) {
            $ids = '';
