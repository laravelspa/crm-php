<?php ob_start();
    session_start();
    $empID = $_POST['emp_id'] !== '' ? $_POST['emp_id'] : $_SESSION['id'];
    ...
    function get_total_orderd_records() {
        ...
        $empID = $_POST['emp_id'] !== '' ? $_POST['emp_id'] : $_SESSION['id']; 
        include('../../main/database.php');
        $query = "SELECT GROUP_CONCAT(orderd_delivery.orderd_id) as d_id, count(*) as count, admins.name as sales_delivery, orderd_delivery.*,orderd_delivery.status as d_status, orderd.* FROM `orderd_delivery` LEFT JOIN orderd ON orderd.id = orderd_delivery.orderd_id LEFT JOIN admins ON admins.id = orderd_delivery.emp_call_id WHERE orderd.city = 'Alexandria' AND orderd_delivery.status = 6 AND orderd_delivery.emp_delivery_id IS NULL GROUP BY orderd.phone";
    ...
    $output = [];
    $query = "SELECT GROUP_CONCAT(orderd_delivery.orderd_id) as d_id, count(*) as count, admins.name as sales_delivery, orderd_delivery.*,orderd_delivery.status as d_status, orderd.* FROM `orderd_delivery` LEFT JOIN orderd ON orderd.id = orderd_delivery.orderd_id LEFT JOIN admins ON admins.id = orderd_delivery.emp_call_id WHERE orderd.city = 'Alexandria' AND orderd_delivery.status = 6 AND orderd_delivery.emp_delivery_id IS NULL";
