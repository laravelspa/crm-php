<?php ob_start();
    ...
    $columnName = [
        ...
        7 => 'orderd.city',
        8 => 'orderd.comment',
        9 => 'orderd_delivery.emp_call_id',
        10 => 'orderd_delivery.d_comment'
    ];
    $output = [];
    $query = "SELECT GROUP_CONCAT(orderd_delivery.orderd_id) as d_id, count(*) as count, a.name as sales_delivery, b.name as delivery, orderd_delivery.*,orderd_delivery.status as d_status, orderd.* FROM `orderd_delivery` LEFT JOIN orderd ON orderd.id = orderd_delivery.orderd_id LEFT JOIN admins as a ON a.id = orderd_delivery.emp_call_id LEFT JOIN admins as b ON b.id = orderd_delivery.emp_delivery_id WHERE orderd.city = 'Alexandria' AND (orderd_delivery.status = 6 OR orderd_delivery.status = 7) AND orderd_delivery.emp_delivery_id IN('" .$ids. "') OR orderd_delivery.emp_call_id IN('" .$ids. "')";
    ...
    if(isset($_POST['search']['value']) && $_POST['search']['value'] != '') {
        ...
        $search .= " OR delivery REGEXP '". $_POST['search']['value'] ."'";
        ...
        $search .= " OR orderd.sales_delivery REGEXP '". $_POST['search']['value'] ."'";
