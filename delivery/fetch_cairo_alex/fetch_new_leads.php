<?php ob_start();
    ...
    function get_total_orderd_records() {
        ...
        $query = "SELECT GROUP_CONCAT(orderd_delivery.orderd_id) as d_id, count(*) as count, a.name as sales, b.name as employee, orderd_delivery.*,orderd_delivery.status as d_status, orderd.* FROM `orderd_delivery` LEFT JOIN orderd ON orderd_delivery.orderd_id = orderd.id LEFT JOIN admins as a ON a.id = orderd.emp_id LEFT JOIN admins as b ON b.id = orderd_delivery.emp_call_id WHERE orderd_delivery.status = 4 AND orderd_delivery.emp_call_id IN('".$imp."') GROUP BY orderd.phone";
    ...
    $output = [];
    $query = "SELECT GROUP_CONCAT(orderd_delivery.orderd_id) as d_id, count(*) as count, a.name as sales, b.name as employee, orderd_delivery.*,orderd_delivery.status as d_status, orderd.* FROM `orderd_delivery` LEFT JOIN orderd ON orderd_delivery.orderd_id = orderd.id LEFT JOIN admins as a ON a.id = orderd.emp_id LEFT JOIN admins as b ON b.id = orderd_delivery.emp_call_id WHERE orderd_delivery.status = 4 AND orderd_delivery.emp_call_id IN('".$imp."')";
    ...
    foreach ($fetch as $row) {
        $sub_array = [];
        if($permission === '1') {
        $sub_array[] = '<input type="checkbox" class="checkedListnew" name="checkedList" value="'.$row['d_id'].'"/>';
        }
