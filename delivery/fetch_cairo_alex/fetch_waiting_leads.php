<?php ob_start();
    ...
    function get_total_orderd_records() {
        ...
        $query = "SELECT GROUP_CONCAT(orderd_delivery.orderd_id) as d_id, count(*) as count, orderd_delivery.*,orderd_delivery.status as d_status, orderd_delivery.created_at as d_create, orderd.* FROM orderd_delivery LEFT JOIN orderd ON orderd_delivery.orderd_id = orderd.id WHERE orderd_delivery.status = 5 AND orderd_delivery.emp_call_id IN('".$imp."') GROUP BY orderd.phone";
    ...
    $output = [];
    $query = "SELECT GROUP_CONCAT(orderd_delivery.orderd_id) as d_id, count(*) as count, orderd_delivery.*,orderd_delivery.status as d_status, orderd_delivery.created_at as d_create, orderd.* FROM orderd_delivery LEFT JOIN orderd ON orderd_delivery.orderd_id = orderd.id WHERE orderd_delivery.status = 5 AND orderd_delivery.emp_call_id IN('".$imp."')";
    ...
    foreach ($fetch as $row) {
        $sub_array = [];
        if($permission === '1') {
        $sub_array[] = '<input type="checkbox" class="checkedListwaiting" name="checkedList" value="'.$row['id'].'"/>';
        }
        $sub_array[] = '<button id="waiting_button" class="btn btn-sm btn-warning" 
                        ...
                        <button id="ready_button" class="btn btn-sm btn-primary" 
                            ...
                            data-ost="'.$row['d_status'].'"
                            data-st="6"
                            data-toggle="modal" data-target="#ReadyModal">Send
                        </button>';
