<?php ob_start();
    ...
    function get_total_orderd_records() {
        ...
        $query = "SELECT GROUP_CONCAT(orderd_delivery.orderd_id) as d_id, count(*) as count, admins.name as sales_delivery, orderd_delivery.*,orderd_delivery.status as d_status, orderd_delivery.created_at as d_create, orderd.* FROM `orderd_delivery` LEFT JOIN orderd ON orderd.id = orderd_delivery.orderd_id LEFT JOIN admins ON admins.id = orderd_delivery.emp_call_id WHERE orderd_delivery.approved = 0 AND orderd_delivery.status = 7 AND orderd_delivery.emp_delivery_id IN('".$imp."') GROUP BY orderd.phone";
    ...
    $output = [];
    $query = "SELECT GROUP_CONCAT(orderd_delivery.orderd_id) as d_id, count(*) as count, admins.name as sales_delivery, orderd_delivery.*,orderd_delivery.status as d_status, orderd_delivery.created_at as d_create, orderd.* FROM `orderd_delivery` LEFT JOIN orderd ON orderd.id = orderd_delivery.orderd_id LEFT JOIN admins ON admins.id = orderd_delivery.emp_call_id WHERE orderd_delivery.approved = 0 AND orderd_delivery.status = 7 AND orderd_delivery.emp_delivery_id IN('".$imp."')";
    ...
    foreach ($fetch as $row) {
        $sub_array = [];
        $sub_array[] = '<button 
                        data-app="2" data-id="'.$row['d_id'].'"
                        data-msg="Successfully arrived order to customer" 
                        data-cmsg="Not know about this order" 
                        id="approved_btn" class="btn btn-outline-success btn-sm mr-1">
                        <i class="fas fa-check"></i>
                        </button>
                        <button 
                        data-app="1" data-id="'.$row['d_id'].'"
                        data-msg="Failed arrived order to customer" 
                        data-cmsg="Not know about this order" 
                        id="canceld_btn" class="btn btn-outline-danger btn-sm">
                        <i class="fas fa-times-circle"></i>
                        </button>';
        $sub_array[] = '<button id="update_button" class="btn btn-sm btn-outline-primary" 
                            data-id="'.$row['d_id'].'"
