<?php include('../../main/database.php');
    function get_total_orderd_records() {
        ...
        $query = "SELECT GROUP_CONCAT(orderd.id) as d_id, count(*) as count, orderd.*, admins.name, as sales FROM orderd LEFT JOIN admins ON orderd.emp_id = admins.id WHERE status = 0 AND city != 'Alexandria' AND city != 'Cairo' GROUP BY orderd.phone";
    ...
    $output = [];
    $query = "SELECT GROUP_CONCAT(orderd.id) as d_id, count(*) as count, orderd.*, admins.name as sales FROM orderd LEFT JOIN admins ON orderd.emp_id = admins.id WHERE status = 0 AND city != 'Alexandria' AND city != 'Cairo'";
