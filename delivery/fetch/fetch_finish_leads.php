<?php ob_start();
    ...
    function get_total_orderd_records() {
        ...
        $query = "SELECT GROUP_CONCAT(orderd_delivery.orderd_id) as d_id, count(*) as count, admins.name as employee, orderd_delivery.*, orderd.*, orderd_delivery.status as d_status FROM orderd_delivery LEFT JOIN admins ON orderd_delivery.emp_call_id = admins.id LEFT JOIN orderd ON orderd_delivery.orderd_id = orderd.id WHERE orderd_delivery.status = 3 AND orderd_delivery.emp_call_id IN('".$imp."') GROUP BY orderd.phone";
    ...
    $output = [];
    $query = "SELECT GROUP_CONCAT(orderd_delivery.orderd_id) as d_id, count(*) as count, admins.name as employee, orderd_delivery.*, orderd.*, orderd_delivery.status as d_status FROM orderd_delivery LEFT JOIN admins ON orderd_delivery.emp_call_id = admins.id LEFT JOIN orderd ON orderd_delivery.orderd_id = orderd.id WHERE orderd_delivery.status = 3 AND orderd_delivery.emp_call_id IN('".$imp."')";
    ...
    $and = '';
    if(isset($date_first) && isset($date_last) && $date_first !== $date_last && $date_first !== '' && $date_last !== '') {
    ...
    if(isset($_POST['search']['value']) && $_POST['search']['value'] != '') {
        ...
        $search .= " OR orderd_delivery.aramex_code REGEXP '". $_POST['search']['value'] ."'";
    ...
    foreach ($fetch as $row) {
        $sub_array = [];
        $sub_array[] = '<input type="checkbox" class="checkedListfinish" name="checkedList" value="'.$row['d_id'].'"/>';
        if($row['approved'] === '0') {
            $sub_array[] = '<button 
                            ...
                            id="approved_btn" class="btn btn-success btn-sm mr-1">
                            ...
                            data-cmsg="Not know about this order" 
                            id="canceld_btn" class="btn btn-danger btn-sm">
