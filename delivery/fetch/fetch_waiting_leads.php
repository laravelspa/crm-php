<?php ob_start();
    ...
    function get_total_orderd_records() {
        ...
        $query = "SELECT GROUP_CONCAT(orderd_delivery.orderd_id) as d_id, count(*) as count, admins.name as employee, orderd_delivery.*,orderd_delivery.status as d_status, orderd_delivery.id as d_id, orderd_delivery.created_at as d_create, orderd.* FROM orderd_delivery LEFT JOIN admins ON orderd_delivery.emp_call_id = admins.id LEFT JOIN orderd ON orderd_delivery.orderd_id = orderd.id WHERE orderd_delivery.status = 1 AND orderd_delivery.emp_call_id IN('".$imp."') GROUP BY orderd.phone";
    ...
    $output = [];
    $query = "SELECT GROUP_CONCAT(orderd_delivery.orderd_id) as d_id, count(*) as count, orderd.*, admins.name as employee, orderd_delivery.*,orderd_delivery.status as d_status, orderd_delivery.created_at as d_create, orderd.* FROM orderd_delivery LEFT JOIN admins ON orderd_delivery.emp_call_id = admins.id LEFT JOIN orderd ON orderd_delivery.orderd_id = orderd.id WHERE orderd_delivery.status = 1 AND orderd_delivery.emp_call_id IN('".$imp."')";
    ...
    foreach ($fetch as $row) {
        $sub_array = [];
        if($permission === '1') {
        $sub_array[] = '<input type="checkbox" class="checkedListwaiting" name="checkedList" value="'.$row['d_id'].'"/>';
        }
