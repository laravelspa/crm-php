    ...
    if($sessionPermission !== '3') {
        ...
        if(isset($sessionName) && isset($sessionPermission) && isset($sessionId)) {
            ...
            foreach ($employees as $employee) {
                $empid = $employee['id'];
                $stmt = $con->prepare("SELECT id, name, permission FROM admins WHERE supervisor = $empid AND permission = 5 AND location = 0");
                $stmt->execute();
                $employees = $stmt->fetchAll(PDO::FETCH_ASSOC);
...
<script src="../js/delivery-supcairo.js"></script>
