    ...
    $(document).on('submit', '#filter_cairo', function (e) {
        ...
        if (count === 1) {
            ...
            if (destination === '#stage') {
                ...
                getRecords('fetch_assistant_cairo/fetch_stage_leads.php', '#stage_orders', emp_id, date_first, date_last, city);
            } else if (destination === '#processing') {
                $('#processing_orders').DataTable().destroy();
                getRecords('fetch_assistant_cairo/fetch_processing_leads.php', '#processing_orders', emp_id, date_first, date_last, city);
            } else {
                $('#approved_orders').DataTable().destroy();
                getRecords('fetch_assistant_cairo/fetch_approved_leads.php', '#approved_orders', emp_id, date_first, date_last, city);
            }
        }
        count = 0;
    })
...
getRecords('fetch_assistant_cairo/fetch_stage_leads.php', '#stage_orders');
getRecords('fetch_assistant_cairo/fetch_processing_leads.php', '#processing_orders');
getRecords('fetch_assistant_cairo/fetch_approved_leads.php', '#approved_orders');
getRecords('fetch_assistant_cairo/fetch_finish_leads.php', '#finish_orders');
...
function getRecords(url, table = '', emp = '', df = '', dl = '', ci = '') {
    ...
    $(table).DataTable({
        ...
        language: {
            ...
            processing: '<i class="fa fa-spinner fa-spin fa-3x fa-fw text-warning"></i><span class="sr-only">Loading...</span> '
        },
...
function updateAll(url, table, msg, emptyMsg, cancelMsg) {
    ...
    formData.append('delivery_man', delivery_man);
