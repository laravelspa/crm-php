<?php 
    ...
    function get_total_orderd_records() {
        include('../main/database.php');
        $query = "SELECT * FROM products_inventory.*, inventory.name INNER JOIN inventory ON products_inventory.inventory_id = inventory.id WHERE inventory.name = 'Alexandria'";
    ...
    $query .= "SELECT products_inventory.*, inventory.name FROM products_inventory INNER JOIN inventory ON products_inventory.inventory_id = inventory.id WHERE inventory.name = 'Alexandria'";
    ...
    foreach ($fetch as $row) {
        ...
        $sub_array[] = '<button type="button" id="edit_project" 
                        ...
                        <button onclick="deleteProduct('."'".$row["id"]."','/delete.php',"."'#table_alex', 'products_inventory'".');" 
